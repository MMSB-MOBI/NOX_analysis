...
cd \$WORKDIR
gunzip -c $inputFilePath > ./database.fasta

hmmsearch $hmmProfileLocation/fad_binding.hmm database.fasta > hmmsearch.out
hmmsearch $hmmProfileLocation/ferric_reduct.hmm database.fasta >> hmmsearch.out
hmmsearch $hmmProfileLocation/nad_binding.hmm database.fasta >> hmmsearch.out
...
i=0
for ifile in `find $DATA_DIR -name "uniprot_sprot_2019_05_v*.fasta.gz"`
    do
    echo $ifile
